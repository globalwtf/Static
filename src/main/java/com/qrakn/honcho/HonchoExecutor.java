...
public class HonchoExecutor {
    ...
    public void execute() {
        try {
                        ...
                        if (methodData.getParameterData().length > 0 && (methodData.getParameterData()[0].getType().equals(CommandSender.class) || methodData.getParameterData()[0].getType().equals(Player.class))) {
                            ...
                            if (!methodData.getParameterData()[0].getType().equals(Player.class) || this.sender instanceof Player) {
                                for (int i = 1; i < parameters.length; ++i) {
                                    ...
                                    final CommandTypeAdapter adapter = this.honcho.getTypeAdapter(parameterData2.getType());
                                    if (adapter == null) {
                                        arguments.add(null);
                                    } else {
                                        Object object;
                                        if (i == parameters.length - 1) {
                                            ...
                                            object = adapter.convert(this.args[i - 1], (Class<Object>) parameterData2.getType());
                                        }
            ...
            e2.printStackTrace();
            this.sender.sendMessage(ChatColor.RED + "There was an error while processing your command...");
        }
    }
    ...
    private String getUsage() {
        ...
        for (final MethodData methodData : this.commandData.getMethodData()) {
            final ParameterData[] parameters = methodData.getParameterData();
            for (int i = 1; i < parameters.length; ++i) {
                final List<String> argument = arguments.getOrDefault(Integer.valueOf(i - 1), new ArrayList<String>());
                final ParameterData parameterData = parameters[i];
                if (parameterData.getType().equals(CommandOption.class)) {
                    arguments.put(Integer.valueOf(i - 1), null);
                } else {
                    ...
                    arguments.put(Integer.valueOf(i - 1), argument);
                }
            }
        }
        for (int j = 0; j < arguments.size(); ++j) {
            final List<String> argument2 = arguments.get(Integer.valueOf(j));
            if (argument2 != null) {
